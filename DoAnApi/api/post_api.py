...
class UserPostDetail(APIView, ApiBase):
    ...
    def put(self, request, slug,*args, **kwargs):
        ...
        serializer = EditPostSerializer(post, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return self.as_success(serializer.data)
        return self.as_errors(serializer.errors)
...
class UserCreatePost(APIView, ApiBase):
    ...
    def create_post(self,user,phone,type,title,video,customer,price,area, province,district, address, decription):
        post = Post(user=user,phone=phone, type=type, title=title,video=video, customer=customer,
               price=price, area = area, province=province,district=district, address=address, decription=decription)
        post.save()
        return post
    ...
    def post(self, request, *args, **kwargs):
        ...
        post = self.create_post(user,phone,type,title,video,customer,price,area, province,district, address, decription)
